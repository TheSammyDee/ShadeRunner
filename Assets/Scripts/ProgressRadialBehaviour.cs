...
namespace ProgressBar
{
    ...
    public class ProgressRadialBehaviour : MonoBehaviour, IIncrementable, IDecrementable
    {
        ...
        void Start()
        {
            ...
            //m_Value = 100;
            //SetFillerSize(100);
            GameController.Instance.chargeBar = this;
            GameController.Instance.SetChargeBar();
        }
        ...
        {
            //if (m_AttachedText)
            //    m_AttachedText.text = Mathf.RoundToInt(fill).ToString();

            m_Fill.fillAmount = fill;
        }

        public void SetChargeText(float num) {
            m_AttachedText.text = Mathf.RoundToInt(num).ToString();
        }
